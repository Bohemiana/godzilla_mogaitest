...
implements Payload {
    ...
    public String getBasicsInfo() {
        ...
        this.systempdir = (String)pxMap.get("systempdir");
        this.gzipDecodeMagic = functions.stringToint((String)pxMap.get("canCallGzipDecode"));
        this.gzipEncodeMagic = functions.stringToint((String)pxMap.get("canCallGzipEncode"));
    ...
    public boolean close() {
        this.isAlive = false;
        ReqParameter reqParameter = new ReqParameter();
        String result = this.encoding.Decoding(this.evalFunc(null, "g_close", reqParameter));
        if ("ok".equals(result)) {
            return true;
        }
        Log.error(result);
